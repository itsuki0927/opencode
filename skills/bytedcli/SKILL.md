...
- 发布任务创建：`devflow`
  - 例子：“创建一个发布任务”“绑定 TCE/TCC 发布项和 Meego”
...
- SQL、数据库平台、数据资产、报表、离线任务：`rds`（子组：`db`/`slow`/`alert`/`ops`/`bpm`）、`bytedoc`、`hive`、`dorado`（子组：`project`/`task`/`instance`/`adhoc`）、`aeolus`、`dataq`、`tqs`
