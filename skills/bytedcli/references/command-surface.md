...
- `self`: bytedcli 自管理能力；当前包含 `self update`（升级全局 npm / pnpm 安装）和 `self tracking`（匿名 agent 使用追踪管理）。

## Code, release, and configuration

- `codebase`: 仓库、MR、Review、Issue、评论、文件、Check Runs、CI、Merge Queue、创建分支。
- `bam`: PSM 列表/搜索、方法列表/详情、版本列表、IDL 更新。子组：`psm`/`method`/`version`/`idl`；旧的平铺命令保留为隐藏别名。
...
- `manta`: Manta 数据探查与两表对比平台；当前覆盖 `auth login`（浏览器登录 DataLeap Manta）、`yarn-queues`（列出用户可用的 YARN 队列）、`profile rule create`（创建数据探查任务）、`comparison job create`（创建两表数据对比任务）和 `comparison sql create`（基于 SQL 的数据比对，自动处理 map/array 字段）；支持 `--region`（cn/sg/va/eu，默认 cn）；探查参数：`--db-name`、`--tb-name`、`--partitions`、`--columns`（省略自动探查全部字段），YARN 队列自动选取；对比参数：`--db-name-old/new`、`--tb-name-old/new`、`--partition-old/new`、`--primary-keys`（JOIN 匹配行，`;` 分隔）、`--comparison-columns`（对比字段，`;` 分隔），YARN 队列自动选取；SQL 比对参数：`--source-table`/`--target-table`（`db.table` 格式）、`--source-filter`/`--target-filter`、`--join-keys`（逗号分隔）、`--map-keys`（map 字段展开 key）、`--dry-run`（仅预览 SQL），YARN 队列自动选取；鉴权使用 DataLeap session cookie（需先 `npm install -g puppeteer-core` 再 `bytedcli manta auth login --region <region>`）；cn 走 `prod` 站点 SSO，sg/va/eu 走 `i18n-tt` 站点 SSO。
- `coral`: Coral 元数据平台；当前覆盖 `ai-generate`，触发 AI 生成资产使用说明文档；`--table-name`、`--operator` 必填，资产全限定名根据表名与区域 cid 自动拼接；`--region`（cn/sg/gcp/va/mycis，默认 sg）、`--generate-type`（默认 ASSET_INSTRUCTION）可选；鉴权优先使用 Session Cookie，也支持 JWT 自动 fallback。
...
- `oneservice`: OneService query 元信息、query version detail，以及按 queryId 自动解析当前 ONLINE version 后提取 SQL；当前覆盖 `meta get --id <queryId>`、`detail get --id <versionId>`、`sql get --id <queryId>`；鉴权依赖 `data.bytedance.net` 的浏览器 session cookie，需先 `bytedcli auth login --session`。
...
- `safe`: 内容治理平台。认证（SSO 或 cookie 登录）、配置管理（tenant/business）、Puzzle 特征平台（feature/entity/datasource/tenant/package/collection 的 list/get）。子组：`puzzle`（含 `puzzle feature`/`puzzle entity`/`puzzle datasource`/`puzzle tenant`/`puzzle package`/`puzzle collection`）；`ds` 是 `datasource` 别名，`pkg` 是 `package` 别名。所有 puzzle 子命令支持 `--tenant` 选项，优先级：`--tenant` > `SAFE_TENANT` env > config > 默认 `ecology`。
...
- `safe.bytedance.net/...` 特征/实体/数据源/租户/包/集合页面：`safe puzzle`
...
- "查 Safe 特征 / 实体 / 数据源 / 租户 / 包 / 集合 / 内容治理平台"：`safe`
