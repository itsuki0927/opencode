...
bytedcli tce cluster scale --cluster-id <cluster_id> --service-id <service_id> --scale-info-file scale.json --yes
...
- **`cluster scale` 安全规则**：与 `cluster update` 相同——(1) ALWAYS 先 `--dry-run`；(2) 等用户确认后再真正提交；(3) 首次提交不要用 `--yes`。`--scale-info-file` 的 JSON 格式为 `{ "runtime": { "replicas": { "dc_info": [...], "canary_dc_info": [...] } } }`
