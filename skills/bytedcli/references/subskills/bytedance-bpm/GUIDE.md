---
name: bytedance-bpm
description: "BPM 流程平台（O2O 项目）: 通过 bytedcli 查询工单、日志、可执行操作，并推进或取消工单。"
---
...
5. `bpm ticket logs`：查看处理日志，确认流转结果
6. `bpm ticket cancel`：兜底场景，需强制关闭未完成工单时使用
...
### 5) 推进工单状态：`bpm ticket update-status`
