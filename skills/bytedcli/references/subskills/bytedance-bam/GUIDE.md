...
| 用户意图 | 路径 | 参考 |
|---------|------|------|
| 搜索 PSM、查方法、查版本、创建 IDL 版本 | **A: BAM 元数据管理** | `references/metadata.md` |
...
- `method gencode` 支持 `--endpoint-id` 或 `--psm` + `--method` 两种定位方式，`--version` 默认为最新版本，`--lang` 默认为 `ts`，`--schema-type` 默认为 `request`，也支持生成 `response`
