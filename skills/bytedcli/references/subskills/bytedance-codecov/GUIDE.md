...
description: "Operate ByteDance codecov via bytedcli: create/update/get/list full coverage reports via the new code-coverage.bytedance.net gateway, print bits coverage-next links, and manage collection-side tags & upload intervals. Use when tasks mention codecov, 代码覆盖率, 覆盖率标签, 覆盖率报告, 全量覆盖率, coverage-next."
---
...
- Create full coverage reports from PSM + branch (primary workflow)
...
```bash
# Report management (code-coverage.bytedance.net, new gateway)
...
# Collection-side (srvcov.byted.org, unchanged)
...
- `codecov report *` 走 `code-coverage.bytedance.net`，与 bits 覆盖率平台新页面 `coverage-next` 对齐
- `codecov create-tag/delete-tag/set-interval` 仍走 `srvcov.byted.org`
