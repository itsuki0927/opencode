...
description: "Operate Manta (DataLeap) data profiling and two-table comparison via bytedcli: create profiling rules, create comparison jobs, authenticate with DataLeap Manta, and manage data quality tasks. Use when tasks mention Manta, DataLeap data profiling, data comparison, data quality checks, or creating profiling/comparison rules."
---
...
Commands are grouped under `manta auth`, `manta yarn-queues`, `manta profile`, and `manta comparison` (including `comparison job` and `comparison sql`).
